 ...
 * @returns 
 */
export function alaBuildSelectTable(fieldName: string, label: string, url: string, columns: any, itemProperty: { [key: string]: any }, rules?: Array<baseRule>, params?: { [key: string]: string }, placeholder?: string, icon?: string, iconWidth?: number, iconHeight?: number, canEmpty = false, singleValue = false, valueToString = false): AlaSelectTableSchema {
    ...
    const obj = alaBuildWithOther("AlaSelectTable", fieldName, label, { url, params, columns: Array.isArray(columns) ? u.tojson(columns) : columns, itemProperty, isFormDesign: false, icon, canEmpty, iconWidth, iconHeight, singleValue, valueToString }, rules, placeholder)
    return obj
}
