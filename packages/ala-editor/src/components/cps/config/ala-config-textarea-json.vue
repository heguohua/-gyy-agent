 ...
 * @LastEditTime: 2025-01-26 11:40:51
 * @FilePath: /1-low-coding/packages/ala-editor/src/components/cps/config/ala-config-textarea-json.vue
 * @Description: 
 * 
 * Copyright (c) 2024 by 【 tech.darcy.zhang@outlook.com 】, All Rights Reserved. 
-->
<template>
    <div class="config-textarea-json">
...
watch(() => formData, (form_data) => {
    if (form_data[key]?.[props.viewport]) {
        logger.info(`bType[ ${bType} ],config-textarea-json组件 【 监听到 】 form_data 发生变化，key[ ${key} ]，即将更新 input 的属性值,input.value=form_data[key][props.viewport]`, form_data[key][props.viewport]);
        input.value = form_data[key][props.viewport] || defaultValue
    } else {
        logger.info(`bType[ ${bType} ],config-textarea-json组件 【 监听到 】 formData 发生变化，key[ ${key} ]，value?.[props.viewport]值不存在,不更新 input.value 属性值`);
    }
}, {
    immediate: true
})
...
watch(input, (value) => {
    ...
    logger.info(`config-textarea-json组件 input 发生变化,即将调用父组件callback, data`, data);
    emit("callback", {
        data: {
            [key]: data
        },
...
watch(() => editorStore.globalParams[bType], () => {
    ...
    if (formData[key]?.[props.viewport]) {
        logger.info(`bType[ ${bType} ],config-textarea-json组件 【 监听到 】 formData 发生变化，key[ ${key} ]，即将更新 input 的属性值,input.value=formData[key][props.viewport]`, formData[key][props.viewport]);
