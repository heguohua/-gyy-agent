 ...
 * @LastEditTime: 2024-12-15 13:26:05
...
watch(formData, (form_data) => {
    ...
    logger.info(`bType[ ${bType} ],【 config-itemProperty组件 监听到 】form_data 发生变化，即将更新 input 的属性值,更新前form_data[key][props.viewport]`, form_data[key]?.[props.viewport]);
...
watch(() => editorStore.globalParams[bType], () => {
    if (formData[key]?.[props.viewport]) {
        logger.info(`bType[ ${bType} ],config-itemProperty组件 【 监听到 】 formData 发生变化，key[ ${key} ]，即将更新 input 的属性值,input.value=formData[key][props.viewport]`, formData[key][props.viewport]);
        input.value = formData[key][props.viewport] || defaultValue
    } else {
        logger.info(`bType[ ${bType} ],config-itemProperty组件 【 监听到 】 formData 发生变化，key[ ${key} ]，value?.[props.viewport]值不存在,不更新 input.value 属性值`);
    }
}, { deep: true })
