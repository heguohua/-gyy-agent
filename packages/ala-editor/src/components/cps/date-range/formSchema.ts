 ...
 * @LastEditTime: 2025-06-03 09:35:46
 * @FilePath: /1-low-coding/packages/ala-editor/src/components/cps/date-range/formSchema.ts
 * @Description: 
 * 
...
import { AlaDateFormat, AlaDateType, AlaDateTypeRange, date } from "@/utils/date";
...
const startFieldName = Type.String({
    code: "config-input",
    title: "开始日期字段名",
    required: true,
    rules: [
...
const endFieldName = Type.String({
    code: "config-input",
    title: "结束日期字段名",
    required: true,
    rules: [
...
const schema = Type.Object({
    ...
    startFieldName: schemaAllViewport(startFieldName),
    endFieldName: schemaAllViewport(endFieldName),
...
export type AlaFormDataRangeSchema = Static<typeof schema>
