 ...
 * @LastEditTime: 2025-10-24 15:05:14
 * @FilePath: /1-low-coding/packages/ala-editor/src/components/cps/select-list-pop/ala-select-list-pop.vue
 * @Description: 
 * 
      ...
      <el-dialog v-model="dialogShow" :width="dialogWidth" :append-to-body="true" :showClose="false">
        ...
        <template #header="{ titleId, titleClass }">
          <div class="ala-list-pop-header">
            <h4 :id="titleId" :class="titleClass">{{ $t('form.p-select-1') }} 【 {{ title }} 】</h4>
          </div>
        </template>

        <div class="dialog-content">
          <!-- 分页列表 -->
          <el-table :data="data" style="width: 100%" :row-key="getRowKeys"
            :default-sort="{ prop: 'id', order: 'descending' }" @selection-change="handleSelectedChange" ref="table"
            v-loading="false">
            ...
            <!-- 主表列渲染 -->
            <el-table-column v-for="column in columns" :key="column.prop" :prop="column.prop" :type="column.type"
