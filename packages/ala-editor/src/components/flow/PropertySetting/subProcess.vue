<template>
  <el-form ref="formRef" :model="form" label-width="130px" size="default">
      <slot name="form-item-subProcess-name" :model="form" field="name">
        <el-form-item label="流程定义名称">
          <el-input v-model="form.name"></el-input>
        </el-form-item>
      </slot>
      <slot name="form-item-subProcess-displayName" :model="form" field="displayName">
        <el-form-item label="流程定义显示名称">
          <el-input v-model="form.displayName"></el-input>
        </el-form-item>
      </slot>
      <slot name="form-item-subProcess-expireTime" :model="form" field="expireTime">
        <el-form-item label="期望完成时间">
          <el-input v-model="form.expireTime"></el-input>
        </el-form-item>
      </slot>
      <slot name="form-item-subProcess-instanceUrl" :model="form" field="instanceUrl">
        <el-form-item label="实例启动Url">
          <el-input v-model="form.instanceUrl"></el-input>
        </el-form-item>
      </slot>
      <slot name="form-item-subProcess-instanceNoClass" :model="form" field="instanceNoClass">
        <el-form-item label="实例编号生成类">
          <el-input v-model="form.instanceNoClass"></el-input>
        </el-form-item>
      </slot>
      <slot name="form-item-subProcess-preInterceptors" :model="form" field="preInterceptors">
        <el-form-item label="节点前置拦截器">
          <el-input v-model="form.preInterceptors"></el-input>
        </el-form-item>
      </slot>
      <slot name="form-item-subProcess-postInterceptors" :model="form" field="postInterceptors">
