 ...
 * @LastEditTime: 2024-11-21 14:49:40
    ...
    <div class="editor-config-render">
        <el-form label-width="auto">
            <div class="" v-for="( item, index ) in list" :key="index" :class="bem('render')" @click="">
                ...
                <component v-if="getComponent(item)" :is="getComponent(item)" :data="item"
                    :viewport="editorStore.viewport[businessType]" @callback="callback"
                    :key="businessType + '-' + item.id" />
            </div>
        </el-form>
    </div>
</template>
...
const getComponent = computed(() => {
    return (item: any) => {
        const code = item.properties[editorStore.viewport[businessType]].code
        const title = item.properties[editorStore.viewport[businessType]].title
        const componentName = alaConsts.COMPONENT_PREFIX + code
        logger.info(`businessType【${businessType}】,editor-config-render中渲染动态 属性表单 组件, code: ${componentName}, title: ${title}`);
...
const callback = (data: any) => {
    logger.info(`businessType【${businessType}】,editor-config-render组件 接收到 子组件callback,即将调用父组件callback, data`, data);
    emit("callback", data)
}
