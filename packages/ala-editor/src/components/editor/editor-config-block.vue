 ...
 * @LastEditTime: 2024-11-20 18:10:37
...
const callback = (params: { data: { cols: any }, id: string }) => {
    ...
    if (!id) {
        logger.error(`【 错误，错误，错误 】接收到 config form item 数据更新，但是其所属组件【 id为空 】，params：`, params);
        return
    }

    const blockConfig = editorStore.blockConfig || []

    logger.info(`editor-config-block组件 接收到 子组件callback,【 即将更新editorStore中的 blockConfig 和 currentSelect 】,nodeId[${id}]，接收到回调参数params：`, params);
    ...
    logger.info(`editor-config-block组件 接收到 子组件callback,【 马上更新editorStore中的 blockConfig 】,nodeId[${id}],更新前editorStore.blockConfig`, editorStore.blockConfig);
    logger.info(`editor-config-block组件 接收到 子组件callback,【 马上更新editorStore中的 blockConfig 】,nodeId[${id}],新的 blockConfig`, newBlockConfig);
    console.log('新的 blockConfig ===>', newBlockConfig);

    editorStore.setBlockConfig(newBlockConfig)

    logger.info(`editor-config-block组件 接收到 子组件callback,【 已更新editorStore中的 blockConfig 】,nodeId[${id}],更新后editorStore.blockConfig`, editorStore.blockConfig);
    ...
    logger.info(`editor-config-block组件 接收到 子组件callback,【 马上更新editorStore中的 currentSelect 】,nodeId[${id}]`);
    ...
    if (currentSelect?.nested && currentSelect.code === 'column') {
        if (data.cols) {
            ...
            if (!oldChildrens) {
                logger.warn(`【 错误，错误，错误 】嵌套对象 currentSelect 中 children 属性不存在，初始化为二维空数组[[],[]]，currentSelect：`, currentSelect);
                oldChildrens = [[], []]
            }
            ...
            logger.info(`currentSelect.childrens原有列数量[ ${oldChildrens.length} ]，新cols列数量[ ${newFormDataCols.length} ]`);
    ...
    if (children && children.length > 0) {
        ...
        logger.warn(`当前currentSelect中【 children 有变化 】，需要更新，更新前 updatedParams：`, updatedParams);
        logger.warn(`当前currentSelect中【 children 有变化 】，需要更新，更新 children：`, children);
        mergeWith(updatedParams, { children }, customMerge)
        logger.warn(`当前currentSelect中【 children 有变化 】，需要更新，更新后 updatedParams：`, updatedParams);
    } else {
        logger.warn(`当前currentSelect中【 children 无变化 】，不需要更新，updatedParams：`, updatedParams);
    }

    logger.info(`editor-config-block组件 接收到 子组件callback,【 马上更新editorStore中的 currentSelect 】,nodeId[${id}],更新前editorStore.currentSelect`, editorStore.currentSelect);
    logger.info(`editor-config-block组件 接收到 子组件callback,【 马上更新editorStore中的 currentSelect 】,nodeId[${id}],更新数据 updatedParams`, updatedParams);
    mergeWith(editorStore.currentSelect, updatedParams, customMerge);
    editorStore.setCurrentSelect(currentSelect)
    logger.info(`editor-config-block组件 接收到 子组件callback,【 已更新editorStore中的 currentSelect 】,nodeId[${id}],更新后editorStore.currentSelect`, editorStore.currentSelect);
