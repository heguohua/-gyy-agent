 ...
 * @LastEditTime: 2025-08-26 10:34:14
...
// ##########################  以下当前模块自定义业务逻辑处理部分  #########################################
const url = '/b/datasetGroup/add'
const updateUrl = '/b/datasetGroup/update'
// 表单数据保存对象
...
const datasetGroupFields = ref<Array<any>>([])

// 基础表单字段
const basicFields = computed(() => {
    return preFields.value.concat(datasetGroupFields.value)
})
...
watch(() => formData['type'], (value: string) => {
    ...
    datasetGroupFields.value = fields

    // datasetGroupField.value = [
...
const beforeSave = async (data: { [key: string]: any }) => {
    ...
    logger.info(`格式化数据后，datasetGroup参数`, d);

    return d
}

const alaBaseForm = ref()
const validateUrl = '/b/datasetGroup/validate'
