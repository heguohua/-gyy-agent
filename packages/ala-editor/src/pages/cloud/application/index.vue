 ...
 * @LastEditTime: 2025-09-14 20:41:28
 * @FilePath: /1-low-coding/packages/ala-editor/src/pages/cloud/application/index.vue
 * @Description: 
 * 
...
<template>
    <div class="page">
        ...
        <div class="left">
            <AlaTree title="业务组" treeUrl="/c/cloudServersGroup/tree" />
        </div>
        <div class="right">
...
import Add from '@/pages/cloud/application/add.vue';
...
const url = application.pageApi
const deleteUrl = "/c/application/delete"

// 分页列表中列属性配置
const columns = computed(() => {
    return application.pageFields
})
/**
 * 详情页面字段
 */
const detailFields: any = ref(application.detailFields)
...
const formAttr = ref(application.formAttr)
...
const handleStart = (row: any) => {
    ...
    alaPost(u.url('/c/application/startOrShutdown'), params, false, '').then((response: any) => {
        let d = response.data
...
const form = ref({ application: undefined, name: undefined })
const rules = {
    name: [
...
const handleConfirm = () => {
    ...
    alaPost(u.url('/c/application/snapshot'), params, false, '').then((response: any) => {
        let d = response.data
...
const handleDelete = (row: any) => {
    ElMessageBox.confirm(
        ...
        .then(() => {
            ...
            alaDelete(u.url('/c/application/delete'), { id: row.id }, false).then((data: any) => {
