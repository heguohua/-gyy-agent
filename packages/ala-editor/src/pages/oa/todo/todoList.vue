 ...
 * @LastEditTime: 2025-10-29 10:40:21
...
<template>
    ...
    <!-- 新增、编辑 -->
    <TodoAdd @refresh="refresh" v-model="showAddForm" :baseInfo="baseInfo" v-if="showAddForm" />
...
const columns = computed(() => {
    return [
        ...
        alaDetailRadio('status', "日程状态", [{ '未开始': 1 }, { '已关闭': 2, 'color': colors.info }, { '已通过': 3, 'color': colors.success }, { '已延期': 4, 'color': colors.danger }, { '进行中': 5, 'color': colors.primary }], undefined, undefined, { columnWidth: { desktop: '150' } }),
...
const detailFields: any = ref([
    ...
    alaDetailDate('endTime', "结束时间", 'YYYY-MM-DD HH:mm', undefined, undefined, { columnWidth: { desktop: '200' } }),
    alaDetailRadio('repeatType', "重复类型", [{ '不重复': 1 }, { '每天': 2, 'color': colors.primary }, { '每周': 3, 'color': colors.success }, { '每半个月': 4, 'color': colors.danger }, { '每一个月': 5 }, { '每2个月': 6 }, { '每3个月': 7 }, { '每半年': 8 }, { '每一年': 9 }, { '每个工作日': 10 }, { '每2天': 11 }, { '每3天': 12 }, { '每4天': 14 }, { '每5天': 15 }, { '每10天': 16 }], undefined, undefined, { columnWidth: { desktop: '150' } }),
    // 待办状态，1-未开始、2-已关闭、3-已通过、4-已延期、5-进行中
    alaDetailRadio('status', "日程状态", [{ '未开始': 1 }, { '已关闭': 2, 'color': colors.info }, { '已通过': 3, 'color': colors.success }, { '已延期': 4, 'color': colors.danger }, { '进行中': 5, 'color': colors.primary }], undefined, undefined, { columnWidth: { desktop: '150' } }),
